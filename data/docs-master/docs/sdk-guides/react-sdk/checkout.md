...
storyManager.on("productCartUpdated", (payload) => console.log(payload));
storyManager.on("productCartClicked", (payload) => console.log(payload));
```
...
#### Example
```ts
storyManager.products.getProductCartState = async () => {
        return {
...
export const App = () => {
    useEffect(() => {
        ...
        storyManager.on("productCartUpdated", handleCartUpdated);
        storyManager.on("productCartClicked", handleCartClicked);
        storyManager.on("closeStoryReader", handleCloseStoryReader);

        return () => {
            storyManager.off("productCartUpdated", handleCartUpdated);
            storyManager.off("productCartClicked", handleCartClicked);
            storyManager.off("closeStoryReader", handleCloseStoryReader);
        };
    }, []);

    return <IASContainer config={{ apiKey: "{projectToken}" }} />
}
```

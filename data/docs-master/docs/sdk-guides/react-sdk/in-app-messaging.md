...
Starting from React SDK `1.4.0` `IAM` was added to the InAppStory SDK.
...
try {
    ...
    const inAppMessaging = storyManager.inAppMessaging;
...
You can change the z-index property of the IAM reader as shown in the example below. Since v1.9.6
...
You can change the maximum width of the toast. Since v1.13.0
...
interface InAppMessaging {
    ...
    showMessageById(messageId: number, options?: { showOnlyIfLoaded?: boolean; signal?: AbortSignal; appearance?: { zIndex?: number, zIndex?: number, toast?: { width?: number } }}): Promise<IamMessage>; // Shows a message by ID

...
```tsx
<IASContainer storyReaderOptions={{
...
| `commonBackdrop`             | object              | [Backdrop options](#backdrop-options) for the StoryReader.                                                                                  |
...
| `navigation`                 | object              | Override navigation buttons SVG icons.                                                                                                      |
...
<IASContainer storyReaderOptions={{
    ...
    timelineBlockTopOffset: 8,
    actionPanelBottomOffset: 0,
    borderRadius: 12,
    layout: StoryReaderLayout.Classic,
}} />
```
...
You can enable/disable controls using `commonOptions` property
...
Displays onboarding stories. [More details...](./onboardings.md)

```tsx
storyManager.showOnboardingStories({ feed: "main", limit: 3 })
...
```tsx
storyManager.showSharePage(123).catch(console.error);
```
...
```tsx
storyManager.showStoryOnce(789);
```

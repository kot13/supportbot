...
> **Note**: If you are using the `CDN` version of `JS SDK`, we have added a compiled `ES5` build `ias.legacy.umd.js` to support legacy browsers.
...
module: {
    rules: [
        {
            test: /\.(js|jsx|ts|tsx)$/,
            exclude: /node_modules[\\\/](?!(@inappstory[\\\/]js-sdk|your-other-module))/,
            use: {
                loader: 'babel-loader',
                options: {
                    presets: [

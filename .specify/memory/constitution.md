...
- Added sections: Core Principles → VI. Спецификации на русском языке
...
- Авторизация/аутентификация проверяется на сервере (не пологаемся на клиент).
...
### VI. Спецификации на русском языке
Вся продуктовая документация фичи в каталоге `specs/` MUST быть на **русском языке**:
- Обязательно: `spec.md`, `plan.md`, `research.md`, `data-model.md`, `quickstart.md`, `tasks.md`, чеклисты в `checklists/`, артефакты в `contracts/` (описательные разделы).
- Пользовательские сценарии, требования, критерии успеха, допущения и пояснения для стейкхолдеров — только на русском.
- Допустимы на английском: идентификаторы кода, пути файлов, имена API/env, фрагменты JSON/CLI и другие технические литералы, неотделимые от реализации.
- Новые и обновляемые спецификации, созданные через `/speckit.specify` и последующие команды, MUST соответствовать этому правилу до merge.

**Rationale**: единый язык снижает барьер для команды и заказчика; исключает расхождения между spec, plan и tasks.
...
- **Определение готовности**:
  ...
  - артефакты `specs/` для фичи соответствуют принципу VI (русский язык).
...
**Version**: 1.1.0 | **Ratified**: 2026-03-31 | **Last Amended**: 2026-07-05
